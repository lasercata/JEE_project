...
class HtmlGenerator {
    ...
    addTableRow(row) {
        ...
        for (const key in this.headers) {
            ...
            // Si c'est un objet ou un tableau, on formate différemment
            if (typeof value === "object" && value !== null) {
                if (key === "schedule") {
                    // Pour un horaire : affiche jour et heure formatée
                    const day = value.day || "-";
                    const start = value.time?.opening ? `${value.time.opening.h}h${String(value.time.opening.m).padStart(2, "0")}` : "";
                    const end = value.time?.ending ? `${value.time.ending.h}h${String(value.time.ending.m).padStart(2, "0")}` : "";
                    value = `${day} ${start} - ${end}`;
                }
                else if (Array.isArray(value)) {
                    // Par exemple personnages
                    if (key === "personnages") {
                        value = value.map(p => p.name).join(", ");
                    }
                    else if (key === "horaires") {
                        value = value
                            .map(
                                s =>
                                `${s.opening.h}h${String(s.opening.m).padStart(2, "0")}–${s.ending.h}h${String(s.ending.m).padStart(2, "0")}`
                            )
                            .join("<br>");
                        td.innerHTML = value;
                        tr.appendChild(td);
        ...
        const setEditHandler = () => {
            ...
            cellRefs.forEach((td, i) => {
                ...
                input.value = Array.isArray(row[key])
                    ? row[key].join(", ")
                    : row[key];
                td.textContent = "";
    ...
    addInputs() {
        ...
        form.action = `/JEE_project/api/${this.title.toLowerCase()}`;
        form.method = 'POST';
        ...
        button.addEventListener("click", () => {
            ...
            // // push to db
            // fetch(`/JEE_project/api/${this.title.toLowerCase()}`, {
            //     method: 'POST',
